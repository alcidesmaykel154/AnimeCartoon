...
exports.GetCubeEmbedLink = function (url) {
  return fetch(url).then((resp) => {
    if ((!resp.ok) || resp.status !== 200) throw Error(`HTTP error! Status: ${resp.status}`)
    return resp.text()
  }).then((data) => {
    const hlsPattern = /source\s+src="([^"]+)"/g
    const matches = hlsPattern.exec(data)
    if (matches && matches[1]) {
      return matches[1]
    } else {
      // Try another pattern if the first one fails
      const m3u8Pattern = /file:\s*"([^"]+\.m3u8[^"]*)"/
      const m3u8Match = data.match(m3u8Pattern)
      if (m3u8Match) return m3u8Match[1]
    }
    console.log("No HLS link found in CubeEmbed")
    return undefined
  })
}
