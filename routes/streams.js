...
function HandleStreamRequest(req, res, next) {
  ...
  if ((videoID?.startsWith("animeflv")) || (videoID?.startsWith("animeav1")) || (videoID?.startsWith("henaojara")) || (videoID?.startsWith("tioanime")) || (videoID?.startsWith("lacartoons"))) { //If we got an AnimeFLV or AnimeAV1 specific ID
    ...
    const lacartoonsp = (videoID?.startsWith("lacartoons")) ? lacartoonsAPI.GetItemStreams(ID) : Promise.resolve([])
    CombineStreams(animeFLVp, animeAV1p, henaojarap, tioanimep, lacartoonsp).then((combinedStreams)=>{
    ...
    animeIMDBIDPromise.then((imdbID) => {
      ...
      const lacartoonsp = lacartoonsAPI.SearchLACartoons(searchTerm).then((animeFLVitem) => {
        const result = fuzzysort.go(searchTerm, animeFLVitem, {key: 'title', limit: 1, all: true})[0]?.obj || animeFLVitem[0];
        if (result) {
...
function CombineStreams(animeFLVPromise, animeAV1Promise, henaojaraPromise, tioanimePromise, lacartoonsPromise) {
  return Promise.allSettled([animeFLVPromise, animeAV1Promise, henaojaraPromise, tioanimePromise, lacartoonsPromise]).then((results) => {
    let combinedStreams = []
    if (results[0].value) {
    ...
    if (results[4].value) {
      console.log(`\x1b[36mGot ${results[4].value.length} LACartoons streams\x1b[39m`)
      lastInternal = combinedStreams.findLastIndex((stream)=>stream.url !== undefined)
      lastInternalLA = results[4].value.findLastIndex((stream)=>stream.url !== undefined)
      if (lastInternalLA === -1) {
        combinedStreams = combinedStreams.concat(results[4].value)
      } else if ((lastInternalLA !== -1) && (lastInternal === -1)) {
        combinedStreams = results[4].value.concat(combinedStreams)
      } else {
        combinedStreams.splice(lastInternal + 1, 0, ...results[4].value.slice(0, lastInternalLA + 1))
        combinedStreams = combinedStreams.concat(results[4].value.slice(lastInternalLA + 1))
      }
    } else {console.error('\x1b[31mFailed on LACartoons slug search because:\x1b[39m ' + results[4].reason)}
