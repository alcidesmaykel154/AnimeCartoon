...
exports.CATEGORIES = [
  { id: 1, name: "Nickelodeon" },
  { id: 2, name: "Cartoon Network" },
  { id: 3, name: "Fox Kids" },
  { id: 4, name: "Hanna Barbera" },
  { id: 5, name: "Disney" },
  { id: 6, name: "Warner Channel" },
  { id: 7, name: "Marvel" },
  { id: 8, name: "Otros" }
];
    ...
    try {
      ...
      $("a[href*='/serie/']").each((i, el) => {
        ...
        if (link && !link.includes("/capitulo/") && !link.includes("/serie/capitulo/")) {
          const title = $(el).find(".nombre-serie").text().trim() || $(el).find("img").attr("alt") || $(el).text().trim();
...
exports.GetByCategory = async function (catId) {
  const url = `${LACARTOONS_BASE}/?Categoria_id=${catId}`;
...
exports.GetShowBySlug = async function (slug) {
  ...
  return fetch(url).then(r => r.text()).then(html => {
    ...
    $(".episodes-container, .episodes, .lista-episodios, .lista-capitulos").find("div, a, li, h4").each((i, el) => {
      ...
      } else if (text.toLowerCase().includes("capitulo") || $(el).attr("href")?.includes("/capitulo/")) {
