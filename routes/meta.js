...
function HandleMetaRequest(req, res, next) {
  ...
  } else if (videoID?.startsWith("lacartoons")) {
    const ID = idDetails[1]
    console.log(`\x1b[33mGot a ${req.params.type} with ${videoID} ID:\x1b[39m ${ID}`)
    lacartoonsAPI.GetShowBySlug(ID).then((animeMeta) => {
      console.log('\x1b[36mGot LACartoons metadata for:\x1b[39m', animeMeta.name)
      res.header('Cache-Control', "max-age=86400, stale-while-revalidate=86400, stale-if-error=259200")
      res.json({ meta: animeMeta, message: "Got LACartoons metadata!" })
      next()
    }).catch((err) => {
      console.error('\x1b[31mFailed on LACartoons slug search because:\x1b[39m ' + err)
      if (!res.headersSent) {
        res.header('Cache-Control', "max-age=86400, stale-while-revalidate=86400, stale-if-error=259200")
        res.json({ meta: {}, message: "Failed getting LACartoons info" });
        next()
      }
    })
  } else {
