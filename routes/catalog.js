...
function HandleCatalogRequest(req, res, next) {
  ...
  } else if (req.params.videoId.startsWith("lacartoons")) {
    //lacartoons catalog request
    if (res.locals.extraParams) {
      catalogPromise = lacartoonsAPI.SearchLACartoons(res.locals.extraParams.search).then((result) => {
        console.log('\x1b[36mGot LACartoons metadata for:\x1b[39m', result.length, "search results")
        return result.map((anime) => {
          return {
